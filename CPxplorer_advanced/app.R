        ...
        shiny::tabPanel("Initial settings",
                        shiny::fluidPage(shiny::sidebarLayout(
                                shiny::sidebarPanel(
                                        ...
                                        shiny::br(),
                                        selectInput("Compclass", "Compound Class",
                                                    ...
                                                    width = NULL,
                                                    size = NULL),
                                        selectInput("Adducts", "Which adduct",
                                                    choices = c("-Cl", "-H", "-HCl", "-Cl-HCl", "-2Cl-HCl", "+Cl","+Br"),
                                                    selected = "-Cl",
                                                    multiple = TRUE,
                                                    selectize = TRUE,
                                                    width = NULL,
                                                    size = NULL),
                                        selectInput("Charge", "Which charge",
                                                    choices = c("-", "+"),
                                                    selected = "-",
                                                    multiple = FALSE,
                                                    selectize = TRUE,
                                        ...
                                        selectInput("TP", "Transformation product?",
                                                    ...
                                                    width = NULL,
                                                    size = NULL),
                                        shiny::numericInput("threshold", "Isotope rel ab threshold (0-99%)", value = 50, min = 0, max = 99),
...
server = function(input, output, session) {
        ...
        selectedCompounds <- shiny::eventReactive(input$go1, {as.character((input$Compclass))})
        selectedAdducts <- shiny::eventReactive(input$go1, {as.character(input$Adducts)})
        selectedCharge <- shiny::eventReactive(input$go1, {as.character((input$Charge))})
        selectedTP <- shiny::eventReactive(input$go1, {as.character((input$TP))})
                ...
                for (i in seq_along(Compounds)) {
                        ...
                        for (j in seq_along(Adducts)) {
                                ...
                                for (k in seq_along(TP)) {
                                        input <- getAdduct(Compounds = Compounds[i], Adduct_Ion = Adducts[j], TP = TP[k], Charge = Charge,
                                                           C = C(), Cl = Cl(), Clmax = Clmax(), Br = Br(), Brmax = Brmax(), threshold = threshold())
