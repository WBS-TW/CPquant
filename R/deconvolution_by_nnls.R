...
library(nnls)
#### TODO ####
#filter(rsquared > 0.9) #cannot do this as some are still false positive and need to replace formula later with 0 for nnls 
...
CPs_standards <- Skyline_output |> 
        ...
        select(-coef) |>  # remove coef variable since it has already been plucked out
        ...
        mutate(rsquared = ifelse(is.nan(rsquared), 0, rsquared)) |> #replace NaN with 0
        #mutate(Response_factor = if_else(rsquared <0.9, 0, rsquared)) |> #replace Response_factor with 0 if rsquared <0.9
...
CPs_samples <- Skyline_output |> 
        filter(`Sample Type` == "Unknown",
               ...
               `Isotope Label Type` == "Quan") |> 
        mutate(Chain_length = paste0("C", str_extract(Molecule, "(?<=C)[^H]+"))) |> 
        #<# choose only SCCPs, this will be remove later or added as arg in fn
        filter(Chain_length == "C10" | Chain_length == "C11" | Chain_length == "C12" | Chain_length == "C13") |> 
        #>#
        group_by(`Replicate Name`) |> 
        mutate(Sample_relative_distribution = `Normalized Area` / sum(`Normalized Area`, na.rm = TRUE)) |> #relative area of each homologue group
        ungroup() |> 
        mutate(across(Sample_relative_distribution, ~replace(., is.nan(.), 0))) #replace NaN with zero

# choose one sample for testing. This will go into a loop to process all samples
CPs_samples_individual <- CPs_samples |> 
        filter(`Replicate Name` == "NIST_R3") |> 
        select(Molecule, Sample_relative_distribution)
...
# The below syntax uses the relative response factor instead of absolute response factor for comparison. Results should be the same?
# CPs_standards_input <- CPs_standards |> 
...
combined_standards_matrix <- combined |> 
        select(-Molecule, -Sample_relative_distribution) |> 
        as.matrix()

combined_sample <- combined$Sample_relative_distribution

# Using the non-negative least squares, nnls, package to deconvolute the distribution
deconv <- nnls(combined_standards_matrix, combined_sample)
...
deconv_reconst <- rowSums(combined_standards_matrix %*% deconv_coef)

chisq.test(deconv_resolved, p = combined_sample, rescale.p = TRUE)

#combine the deconvolution results with the original standards and sample
results <- deconv_reconst |> as_tibble() |> 
        cbind(combined) |> 
        rename(Reconstructed_relative_distribution = value) |> 
        select(Molecule, everything()) |> 
        mutate(across(-Molecule, ~./sum(.x))) |>  #recalculate to rel distr (sum of column = 1)
        mutate(Molecule = as.factor(Molecule))

factors <- as.character(results$Molecule) #refactor according to original order
results_long <- results |> pivot_longer(cols = -Molecule, names_to = "Sample", values_to = "rel_distr") |> 
        mutate(Molecule = as.factor(Molecule)) |> 
        mutate(Molecule = fct_relevel(Molecule, factors))

#plot all sample, standard and reconstructed 
results_long |> 
        ggplot(aes(x = Molecule, y = rel_distr, fill = Sample)) + 
        geom_bar(stat = "identity") +
        theme(axis.text.x = element_text(angle=90)) +
        facet_wrap(~Sample)

# plot only sample and reconstructed
results_long |> 
        filter(Sample == "Sample_relative_distribution" | Sample == "Reconstructed_relative_distribution") |> 
        ggplot(aes(x = Molecule, y = rel_distr, fill = Sample)) + 
        geom_bar(stat = "identity", position = position_dodge()) +
        theme(axis.text.x = element_text(angle=90))

# calculate concentrations by relative response factors
